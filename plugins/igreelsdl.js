...
module.exports = {
  name: "igreelsdl",
  desc: "instagram reels downloader",
  cmd: "igrdl",
  async execute(ctx, m, exe) {
    try {
        
        if (!exe) return m.reply("mana url ig nya");
        let donglod = await fetch(
          `${global.api}/igreelsdl?url=${exe}`
        );
        let result = await donglod.json();
        if (!result.success) return m.reply("gagal mendownload");
        return await ctx.sendVideo(m.chat, result.data.downloadUrl, "", m);
